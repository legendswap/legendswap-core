// Dependency file: contracts/interfaces/ILegendSwapRouter01.sol
...
// Dependency file: contracts/interfaces/ILegendSwapRouter02.sol

// pragma solidity >=0.6.2;

// import './ILegendSwapRouter01.sol';
...
// import './interfaces/ILegendSwapRouter02.sol';
...
contract LegendSwapRouter is ILegendSwapRouter02 {
    ...
    function removeLiquidity(
        ...
        address pair = LegendSwapLibrary.pairFor(factory, tokenA, tokenB);
        ILegendSwapPair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
    ...
    function removeLiquidityBNBSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransfer(token, to, IBEP20LegendSwap(token).balanceOf(address(this)));
        IWBNB(WBNB).withdraw(amountBNB);
        TransferHelper.safeTransferBNB(to, amountBNB);
    }
    function removeLiquidityBNBWithPermitSupportingFeeOnTransferTokens(
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            amountInput = IBEP20LegendSwap(input).balanceOf(address(pair)).sub(reserveInput);
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        uint balanceBefore = IBEP20LegendSwap(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IBEP20LegendSwap(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        uint amountOut = IBEP20LegendSwap(WBNB).balanceOf(address(this));
