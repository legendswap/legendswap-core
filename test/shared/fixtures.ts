...
import LegendSwapFactory from '../../build/LegendSwapFactory.json'
import ILegendSwapPair from '../../build/ILegendSwapPair.json'
import LegendSwapRouter from '../../build/LegendSwapRouter.json'
...
export async function factoryFixture(_: Web3Provider, [wallet]: Wallet[]): Promise<FactoryFixture> {
  const factory = await deployContract(wallet, LegendSwapFactory, [wallet.address], overrides)
  return { factory }
}
...
export async function pairFixture(provider: Web3Provider, [wallet]: Wallet[]): Promise<PairFixture> {
  ...
  const pair = new Contract(pairAddress, JSON.stringify(ILegendSwapPair.abi), provider).connect(wallet)
  ...
  const WBNBPair = new Contract(WBNBPairAddress, JSON.stringify(ILegendSwapPair.abi), provider).connect(wallet)
...
export async function routerFixture(provider: Web3Provider, [wallet]: Wallet[]): Promise<RouterFixture> {
  ...
  const router = await deployContract(wallet, LegendSwapRouter, [factory.address, WBNB.address], overrides)
